...
describe('ProductsList.test', () => {
  ...
  test('Test render ProductsList', () => {
    componentRender(<ProductsList products={products} />);
    const productList = screen.getByTestId('productList');
    expect(productList).toBeInTheDocument();
    expect(productList).toHaveTextContent('Шампунь beauty-full volume плотность и объем');
    expect(productList).toContainHTML('product4.webp');
    const productsImg = screen.getAllByAltText('product');
    expect(productsImg.length).toBe(products.length);
    const typesizeImg = screen.getAllByAltText('typesize');
    expect(typesizeImg.length).toBe(products.length);
  });
});